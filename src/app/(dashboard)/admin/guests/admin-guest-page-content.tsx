...
import { AdminGuestsTable } from "@/components/admin-guest-table";
import { Card, CardContent, CardHeader, CardTitle } from "@/components/ui/card"
import { fetchAllGuestStats } from "@/lib/queries";
import { useQuery } from "@tanstack/react-query"
import { ClipboardList, LogOut, Users } from "lucide-react"



export const AdminGuestPageContent = () => {
    const { data: stats, isLoading, error, isError } = useQuery({
        queryKey: ['guests-stats'],
        queryFn: fetchAllGuestStats,
        refetchOnWindowFocus: true,
        refetchOnMount: true,
        staleTime: 1000,
      });
    console.log("Data from Pge" , stats)
    console.log('Guest render state:', { stats, isLoading, error })
    if (isLoading) {
        return (
            <div className="p-6">
