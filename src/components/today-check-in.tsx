...
import { fetchTodayCheckInGuests } from "@/lib/queries"
import { Guest } from "@/types/guests"

export function TodayCheckInTable() {
      ...
      },
      {
        ...
        cell: ({ row }) => {
          ...
          return (
            <Button
              variant="ghost"
              onClick={() => router.push(`/admin/guests/${guest._id}`)}
            >
              View Details
            </Button>
          )
  ...
  const { data: guests = [], isLoading, error } = useQuery({
    queryKey: ['today-check-in-guests'],
    queryFn: fetchTodayCheckInGuests,
    refetchOnWindowFocus: true,
    refetchOnMount: true,
    staleTime: 1000, 
  });

  console.log("Fetched guests:", guests); // Debug log
    ...
    <div className="w-full">
      <div className="flex items-center py-4 gap-2">
        <Input
          ...
          className="max-w-sm bg-white text-black shadow-md"
        />
        <DropdownMenu>
