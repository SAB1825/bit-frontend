...
import { fetchTodayCreatedComplaints } from "@/lib/queries"
...
export function TodayCreatedComplaintsTable() {
      ...
      },
      {
        ...
        cell: ({ row }) => {
          ...
          return (
            <Button
              variant="ghost"
              onClick={() => router.push(`/staff/complaints/${complaint.complaintId}`)}
            >
              View
            </Button>
          )
  ...
  const { data: complaints = [], isLoading, error } = useQuery({
    queryKey: ['today-created-complaints'],
    queryFn: fetchTodayCreatedComplaints,
  })
